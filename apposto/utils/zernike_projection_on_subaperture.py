...
NB: Negro84 doesn't use the some ordering of Zernike of Noll76. This
class reorder according to Noll76

BUG: something wrong in computed values z2,z3 when z7 is not null in the full pupil
"""
...
class ZernikeProjectionOnSubaperture(object):
    ...
    def _computeMatrix(self):
        ...
        z7= np.array([3* np.sqrt(2) * hoR**2 * roR * s2a,
                      3* np.sqrt(2) * roR * (hoR**2 * (1+ 2*sa**2) + 2./3*roR**2 - 2./3),
        ...
        z11= np.array([12 * np.sqrt(5) * hoR * roR * (hoR**2 + 2./3 * roR**2 - 0.5) * ca,
                       12 * np.sqrt(5) * hoR * roR * (hoR**2 + 2./3 * roR**2 - 0.5) * sa,
