...
class ModalDecomposer(object):
    ...
    def __init__(self, n_modes, modeType='zernike', keepMatrices=True):
        self._nModes = n_modes
        self._modesType = modeType
        self._

    @cacheResult
    def _synthRecFromSlopes(self, nModes, circular_mask):
    ...
    def measureCoefficientsFromSlopes(self, slopes, mask, nModes=None):
        if nModes is None:
        ...
        reconstructor = self._synthRecFromSlopes(nModes, mask)
    ...
    @cacheResult
    def _synthRecFromWavefront(self, nModes, circular_mask,
                                      user_mask):
        zg = ZernikeGenerator(circular_mask)
    ...
    @returns(ModalCoefficients)
    def measureCoefficientsFromWavefront(self,
                                                wavefront,
                                                circular_mask,
                                                user_mask,
                                                nModes=None):
        ...
        self._reconstructor = self._synthRecFromWavefront(nModes,
                                                           circular_mask,
                                                           user_mask)
        ...
        return ModalCoefficients.fromNumpyArray(
            np.dot(wavefrontInMaskVectorNoPiston, self._reconstructor))
